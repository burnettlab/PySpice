...
for voltage in (dc_offset - ac_amplitude, dc_offset, dc_offset + ac_amplitude):
    ...
    # Fixme: handle unit
    assert len(analysis.out) == 1
    quiescent_voltage = float(analysis.out[0])
    assert len(analysis.Vinput) == 1
    ...
    print(
        "Quiescent Point {:.1f} mV {:.1f} mA".format(
...
dynamic_resistance = (
    quiescent_points[0]["quiescent_voltage"] - quiescent_points[-1]["quiescent_voltage"]
) / (
    quiescent_points[0]["quiescent_current"] - quiescent_points[-1]["quiescent_current"]
)
...
analysis = simulator.transient(
    step_time=source.period / 1e3, end_time=source.period * 4
)
...
ax3.legend(("Vin [V]", "Vout [V]"), loc=(0.8, 0.8))
ax3.grid()
ax3.set_xlabel("t [μs]")
ax3.set_ylabel("[V]")
